...
test("plan --help lists skill recommendation and dry-run options", () => {
  ...
  assert.match(result.stdout, /--max-files/);
...
test("plan passes model option and respects max visible files", () => {
  const fixture = createSkillFixture();
  fs.writeFileSync(path.join(fixture.workspace, "first.txt"), "one", "utf8");
  fs.writeFileSync(path.join(fixture.workspace, "second.txt"), "two", "utf8");
  const promptFile = path.join(
    fs.mkdtempSync(path.join(os.tmpdir(), "claude-plugin-options-")),
    "prompt.txt"
  );
  const argsFile = path.join(path.dirname(promptFile), "args.txt");
  const fake = createFakeClaude({
...
exit 9`
  ...
  const result = runNode(["plan", "--model", "claude-sonnet-4-5", "--max-files", "1", "Check files"], {
  ...
  assert.equal(result.status, 0);
  assert.match(fs.readFileSync(argsFile, "utf8"), /--model claude-sonnet-4-5/);
  const prompt = fs.readFileSync(promptFile, "utf8");
  const visibleFiles = prompt.split("Visible files:")[1].trim().split(/\r?\n/).filter(Boolean);
  assert.equal(visibleFiles.length, 1);
});

test("plan --output writes a metadata-wrapped plan file", () => {
  const fixture = createSkillFixture();
  const outputPath = path.join(fixture.workspace, "PLAN.md");
  const fake = createFakeClaude({
    body: `if [ "$1" = "--version" ]; then
  echo "claude 9.9.9"
  exit 0
...
exit 9`
  ...
  const result = runNode(["plan", "--skill", "writing-plans", "--output", outputPath, "Save plan"], {
    cwd: fixture.workspace,
    env: {
      ...process.env,
      HOME: fixture.home,
  ...
  assert.equal(result.status, 0);
  assert.match(result.stdout, /Plan saved:/);
  const saved = fs.readFileSync(outputPath, "utf8");
  assert.match(saved, /Generated by claude:plan/);
  assert.match(saved, /Request: Save plan/);
  assert.match(saved, /Skills: writing-plans/);
...
exit 9`
  ...
  const result = runNode(["plan", "--timeout", "1", "Check setup"], {
    env: {
      ...process.env,
      PATH: `${fake.binDir}${path.delimiter}${process.env.PATH}`,
      CLAUDE_PLUGIN_ROOT: repoRoot
...
test("plan --dry-run prints prompt without calling Claude", () => {
  ...
  const result = runNode(["plan", "--dry-run", "--model", "claude-sonnet-4-5", "--show-skills", "Need frontend planning"], {
    cwd: fixture.workspace,
    env: {
      ...process.env,
      HOME: fixture.home,
...
test("plan --check accepts a valid plan file", () => {
  const fixture = createSkillFixture();
  const planPath = path.join(fixture.workspace, "VALID_PLAN.md");
  fs.writeFileSync(
...
`,
  ...
  const result = runNode(["plan", "--check", planPath], {
    cwd: fixture.workspace,
    env: {
      ...process.env,
      HOME: fixture.home,
  ...
  assert.equal(result.status, 0);
  assert.match(result.stdout, /Claude plan check/);
  assert.match(result.stdout, /Result: ok/);
});

test("plan --check reports missing sections, unsafe commands, unknown files, and missing scripts", () => {
  const fixture = createSkillFixture();
  const planPath = path.join(fixture.workspace, "BAD_PLAN.md");
  fs.writeFileSync(
    planPath,
    `Summary
...
`,
  ...
  assert.equal(result.status, 4);
  assert.match(result.stdout, /Missing required sections: Current Understanding, Risks/);
  assert.match(result.stdout, /Destructive command detected: rm -rf/);
  assert.match(result.stdout, /Unknown npm script: missing/);
  assert.match(result.stdout, /Referenced path does not exist: src\/missing\.ts/);
});
