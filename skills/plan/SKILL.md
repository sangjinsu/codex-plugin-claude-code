...
argument-hint: "[--list-skills] [--recommend-skills] [--check file] [--dry-run] [--output file] [--model name] [--timeout ms] [--max-files n] [--show-skills] [--skill id] [--skills id,id] [request]"
---
...
- `--check <file|->`: validate a plan file or stdin without running Claude.
- `--dry-run`: print the final prompt/context without running Claude.
- `--output <file>`: save the returned plan with metadata.
- `--save`: save the returned plan to `PLAN.md`.
